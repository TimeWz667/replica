...
theme_set(theme_tidybayes() + theme(text=element_text(family="sens")))

sims_base <- read_csv("out/vesga2019/Runs_Prior.csv") %>% 
...
#  filter(Time > 2008) %>% 
  ggplot() +
  stat_lineribbon(aes(x = Time, y = PrevTB), .width = c(.99, .95, .8, .5), color = "#08519C") +
  geom_line(aes(x = Time, y = PrevTB, group = Key), alpha = 0.05) +
...
sims_base %>% 
  ...
  stat_lineribbon(aes(x = Time, y = PrevTB), .width = c(.99, .95, .8, .5), color = "#08519C") +
  geom_line(aes(x = Time, y = PrevTB, group = Key), alpha = 0.05) +
  scale_y_continuous("Prevalence", labels = scales::label_number(scale = 1E5)) +
  scale_x_continuous("Year") +
  scale_fill_brewer()



#sims_ctrl <- read_csv("out/Post_Original/Runs_Ctrl.csv") %>% mutate(Scenario = "Base")
#sims_intv <- read_csv("out/Post_Original/Runs_Intv.csv") %>% mutate(Scenario = "Intv")
...
sims <- sims_base %>% 
  select(Time, Prevalence = PrevTB, Incidence = IncTB, Mortality = MorTB) %>% 
  group_by(Time) %>% 
  ...
  pivot_wider(c(Time, Index), names_from = Stats, values_from = value) %>% 
  mutate(
...
sims_base <- read_csv("out/Prior_Original/Runs.csv") %>% 
  mutate(Scenario = "Prior")
...
ggsave(g_frac, filename = here::here("docs", "vesga2019", "Prior_FracPrev.png"), width = 6, height = 4)
ggsave(g_epi, filename = here::here("docs", "vesga2019", "Prior_Epi.png"), width = 6, height = 4)
